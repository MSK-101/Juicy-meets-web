...
export default function FixedVideoChatPage() {
  ...
  // Streams - Fix: Proper stream state management
  ...
  const handleSwipeToNext = async () => {
    try {
      // DON'T reset streams immediately - keep them for persistence
      ...
      // Force immediate stream refresh
      setTimeout(() => {
        refreshStreams();
        setStreamVersion(prev => prev + 1); // Force re-render
      }, 200); // Faster refresh
  ...
  // =============================================================================
  // STREAM MANAGEMENT - FIX FOR GRAY SCREEN
  // =============================================================================
  ...
  const refreshStreams = useCallback(() => {
    try {
      ...
      // Get fresh remote stream
      const currentRemoteStream = cleanVideoChatService.getCurrentRemoteStream();
  ...
  // Setup callbacks early to prevent timing issues
  useEffect(() => {
    if (!isClient) return;

    // Remote stream callback - FIXED: Force re-render with version
    ...
    // Local stream will be retrieved via getCurrentLocalStream()
  ...
  const initializeVideoChat = useCallback(async () => {
    ...
    try {
      ...
      // FIXED: Force local stream refresh on initialization
      setTimeout(() => {
        refreshStreams();
      }, 1000);
      ...
      cleanVideoChatService.onConnectionStateChange((state) => {
        if (state === 'connected') {
          setConnectionState('connected');
          // Force stream refresh when connected
          setTimeout(() => {
            refreshStreams();
          }, 500);
  ...
  }, [isClient, isInitialized, isConnecting, userId, messages, refreshStreams]);
  ...
  // =============================================================================
  // STREAM EFFECTS - FIXED WITH VERSION TRACKING
  // =============================================================================

  useEffect(() => {
    if (localStream && localVideoRef.current) {
      // Force srcObject assignment even if same stream
  ...
  // Ultra-fast stream refresh to prevent gray screen
  useEffect(() => {
    ...
    }, 1000); // Check every second for ultra-fast recovery

    return () => clearInterval(interval);
  }, [isClient, isInitialized, connectionState, isVideoPlaying, refreshStreams]);
        ...
        <div className="flex-1 relative">
          {isVideoPlaying && currentVideoUrl ? (
            ...
            <div className="relative w-full h-full bg-black">
              {/* Remote video - FIXED: Key prop forces re-render */}
              <video
                key={`remote-${streamVersion}`}
                ref={remoteVideoRef}
              ...
              {/* Local video overlay - FIXED: Key prop forces re-render */}
              {localStream && (
                <div className="absolute top-4 right-4 w-32 h-24 bg-gray-800 rounded-lg overflow-hidden">
                  <video
                    key={`local-${streamVersion}`}
                    ref={localVideoRef}
                    className="w-full h-full object-cover"
                    autoPlay
                    playsInline
              ...
              {/* No stream indicator */}
              {!localStream && !remoteStream && connectionState === 'connected' && (
